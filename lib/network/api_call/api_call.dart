...
class ApiCall {
  static Future postApiCalltoken(
      String url, model, String token, String businessId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (response.statusCode == 401) {
          ...
          try {
            ...
            MyToast.toast("$e");
            return {
              "error": "Unauthorized access, but failed to parse response."
            };
          }
  ...
  static Future postApiCallDownload(
      String url, model, String token, String businessId) async {
    try {
      final response = await http.post(
        Uri.parse('${MyUrls.BASE_URL}/$url/$businessId'),
  ...
  static Future postFormDataApiCall(String url, Map<String, dynamic> formData,
  ...
  static Future getApiCall(String url, String token, String id,
      [String? yes]) async {
  ...
  static Future getApiCallWithoutId(String url, String token,
      [String? yes]) async {
    String? showToast = yes;
    try {
  ...
  static Future getApiCallParam(
      ...
      String endDate = '',
      String? yes,
      String? urlIdentifier}) async {
    String? showToast = yes;
    String url = '';
    try {
      if (urlIdentifier == "0") {
        url =
            '${MyUrls.BASE_URL}/$endpoint/$id?query=${query ?? ''}&page=$page&size=$size&sort=$sort&args=$args&start_date=$startDate&end_date=$endDate';
      } else if (urlIdentifier == '2') {
        url =
            '${MyUrls.BASE_URL}/$endpoint/$id?query=${query ?? ''}&page=$page&size=$size&sort=$sort&args=$args&startdate=$startDate&enddate=$endDate';
      } else {
        url =
            '${MyUrls.BASE_URL}/$endpoint/$id?query=${query ?? ''}&page=$page&size=$size&sort=$sort&args=$args';
      }
      debugPrint("-----------------raj-${url}---------------");
      final response = await http.get(
        Uri.parse(url),
