...
class AllInvoiceController extends GetxController {
  ...
  List<ResTransactionDetail> allInvoiceList =
  ...
  Future<void> downloadCSV() async {
    try {
      ...
      for (var transaction in allInvoiceList) {
      ...
      if (status.isGranted) {
        ...
        if (downloadsDirectory != null) {
          final filePath = "${downloadsDirectory.path}/AllInvoice.csv";
          ...
          await flutterLocalNotificationsPlugin.show(
            0,
            'Download Complete',
            'Tap to open the AllInvoice CSV file.',
  ...
  getAllInvoiceData(
    ...
    try {
      ...
      if (res != null) {
        ...
        allInvoiceList.addAll(customerList);

        if (allInvoiceList.isEmpty) {
