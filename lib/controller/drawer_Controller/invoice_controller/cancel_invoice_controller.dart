...
class CancelInvoiceController extends GetxController {
  ...
  List<ResTransactionDetail> cancelInvoiceList =
  ...
  Future<void> downloadCSV() async {
    try {
      ...
      for (var transaction in cancelInvoiceList) {
      ...
      if (status.isGranted) {
        ...
        if (downloadsDirectory != null) {
          final filePath = "${downloadsDirectory.path}/cancelInvoice.csv";
          ...
          await flutterLocalNotificationsPlugin.show(
            0,
            'Download Complete',
            'Tap to open the cancelInvoice CSV file.',
  ...
  getAllInvoiceData(
    ...
    try {
      ...
      if (res != null) {
        ...
        cancelInvoiceList.addAll(customerList);

        if (cancelInvoiceList.isEmpty) {
