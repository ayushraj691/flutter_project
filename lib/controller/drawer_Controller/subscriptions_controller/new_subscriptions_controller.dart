...
class NewSubscriptionsController extends GetxController {
  var variableController = Get.find<VariableController>();

  List<ResTransactionDetail> newSubscriptionsList =
  ...
  Future<void> downloadCSV() async {
    try {
      final List<List<String>> csvRows = [
        ['TransactionId', 'Customer', 'Amount', 'Created On', 'Mode', 'status']
      ];

      for (var transaction in newSubscriptionsList) {
      ...
      if (status.isGranted) {
        ...
        if (downloadsDirectory != null) {
          final filePath = "${downloadsDirectory.path}/newSubscription.csv";
          ...
          await flutterLocalNotificationsPlugin.show(
            0,
            'Download Complete',
            'Tap to open the newSubscription CSV file.',
  ...
  verifyData(List<String> ids) async {
    ...
    ReqVerifyTransactionData reqVerifyTransactionData =
        ReqVerifyTransactionData(ids: ids);
    debugPrint(json.encode(reqVerifyTransactionData.toJson()));
    var res = await ApiCall.postApiCalltoken(
        MyUrls.verifyPayment,
        reqVerifyTransactionData,
  ...
  getAllSubscriptionsData(
    ...
    try {
      newSubscriptionsList.clear();

      var res = await ApiCall.postApiCallParam(
          endpoint: MyUrls.allSubscriptionDetail,
          id: businessId,
      ...
      if (res != null) {
        ...
        newSubscriptionsList.addAll(customerList);

        if (newSubscriptionsList.isEmpty) {
          // MyToast.toast("No subscription found.");
        }
      } else {
