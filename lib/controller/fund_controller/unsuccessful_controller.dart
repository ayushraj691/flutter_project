...
class UnsuccessfulFundsController extends GetxController {
  var variableController = Get.find<VariableController>();
  List<ResFundsDetails> unSuccessfulFundsList =
      List<ResFundsDetails>.empty(growable: true).obs;

  final startDate = ''.obs;
  final endDate = ''.obs;

  void callMethod() async {
    Map<String, dynamic> sortMap = {"is_created": -1};
    Map<String, dynamic> argumentMap = {
      "is_approved": "4",
    };
    await getUnsuccessfulFundsData(
  ...
  Future<void> downloadCSV() async {
    try {
      final List<List<String>> csvRows = [
        ['FundSource', 'Amount', 'status', 'Created On']
      ];

      for (var transaction in unSuccessfulFundsList) {
        final createdDate = transaction.allfunds?.isCreated;
        DateTime dateTime;
        try {
        ...
        csvRows.add([
          "${transaction.fundSourcedetail?.name}",
          "\$${transaction.allfunds!.addedAmount}",
          transaction.allfunds?.isApproved == "0"
              ? "Pending"
              : transaction.allfunds?.isApproved == "3"
                  ? "Successful"
      ...
      if (status.isGranted) {
        ...
        if (downloadsDirectory != null) {
          final filePath = "${downloadsDirectory.path}/unsuccessfulFunds.csv";
          ...
          await flutterLocalNotificationsPlugin.show(
            0,
            'Download Complete',
            'Tap to open the unsuccessfulFunds CSV file.',
  ...
  getUnsuccessfulFundsData(
    ...
    try {
      unSuccessfulFundsList.clear();

      var res = await ApiCall.getApiCallParam(
          endpoint: MyUrls.allFilterFunds,
          id: businessId,
      ...
      if (res != null) {
        ...
        List<ResFundsDetails> fundsList = JsonUtils.parseCustomerData(res);
        unSuccessfulFundsList.addAll(fundsList);

        if (unSuccessfulFundsList.isEmpty) {
...
class JsonUtils {
  static List<ResFundsDetails> parseCustomerData(dynamic jsonResponse) {
    if (jsonResponse is String) {
      final parsed = jsonDecode(jsonResponse);
      return (parsed['data'] as List)
          .map((json) => ResFundsDetails.fromJson(json))
          .toList();
